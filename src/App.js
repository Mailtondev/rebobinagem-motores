  ...
  const handleAddRebobinagem = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        fotoFile: null, fotoPreview: null, fotoURL: '',
        dataServico: new Date().toISOString().split('T')[0]
      });
      setShowForm(false);
      setEditingId(null);
          ...
          {showForm && (
            <div className="bg-gradient-to-br from-blue-50 to-cyan-50 p-8 border-b-4 border-blue-300">
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => {setShowForm(false); setEditingId(null); setFormData({cliente: '', cv: '', polos: '', marca: '', fotoFile: null, fotoPreview: null, fotoURL: '', dataServico: new Date().toISOString().split('T')[0]});}} className="bg-gray-400 text-white px-8 py-3 rounded-lg flex items-center gap-2 font-bold">
                  <X size={20} /> Cancelar
                </button>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-2xl max-w-2xl w-full">
            ...
            <div className="flex justify-center p-6 border-t">
              <button onClick={() => setFotoModal(null)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg font-bold">Fechar</button>
            </div>
          </div>
        </div>
      )}
